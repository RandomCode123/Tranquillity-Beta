...
- ## typeToken: 
    该属性表示所属标签的类型，该属性为**必要标签**. 该属性有**sentence**和**codeBlock**两个可设值. <br>
    若可设值为sentence，则表示所属标签在代码中属于在<b>语句(Sentence)</b>中出现的令牌； 若可设值为codeBlock，则表示所属标签在代码中属于<b>代码块(Code Block)</b>中出现的令牌. 
- ## rangeDirection: 
    该属性表示所属标签所要包含的<b>解析代码(Processing Code)</b>相对于所属标签的方向，该属性为**必要标签**. 该属性有**left**, **bothSide**和**right**三个可设值. <br>
